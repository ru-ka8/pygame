...
resized_car1_image = pygame.transform.smoothscale(car1_image, [20, 20])
screen.blit(resized_car1_image, (x, y)) # draw the car image at the given position
...
#def draw_car(x, y): # now only takes x and y
    #car1_image = pygame.image.load("car_1.png").convert_alpha()
    #resized_car1_image = pygame.transform.scale(car1_image, [20, 20])
    #screen.blit(resized_car1_image, (x, y)) # draw the car image at the given position

#kokokara!! car wp draw suru

def game_loop():
    quit_game = False
    game_over = False  

    x = 250   # x-coordinate for snake
    y = 100   # y-coordinate for snake

    x_change = 0  # Holds value of change in snake x-coordinate
    y_change = 0  # Holds value of change in snake y-coordinate
