...
def load_high_score():
    # Checks if HI_score.txt file exists. If it doesn't, creates file and writes the value 0 to the file.
    # Then it sets the variable named value as the value read from the file and returns it.
    # .txt can save the data to a file for later use
    try:
      hi_score_file = open("HI_score.txt", 'r') # open it in read-only
    except:
      hi_score_file = open("HI_score.txt", 'w') # open file in write mode. if file does not exist, creates file
      hi_score_file.write("0") # writes content to file
...
def update_high_score(score, high_score):
    ...
    # if so, updates high score. Otherwise, high score is returned as updated high score.
...
def display_scores(score, score_colour, high_score):
    ...
    # Current score:
    display_score = score_font.render("SCORE: " + str(score), True, score_colour)
    screen.blit(display_score, (400, 50))
    # str() is used to convert an object to a string

    # High score:
    display_score = score_font.render("HIGH SCORE: " + str(high_score), True, score_colour)
...
# class is used like dictionary 
class Cars(pygame.sprite.Sprite): # move an object independently of the bg
...
class PlayersCar(Cars):
    ...
    def __init__(self, x, y):
        car1 = pygame.image.load('Assesment/car_1.png').convert_alpha()
...
def game_loop():
  ...
  image_name = ['car_2.png', 'car_3.png', 'car_4.png', 'car_5.png', 'car_6.png']
  car_images = []
  for image_name in image_name:
    image = pygame.image.load('Assesment/' + image_name).convert_alpha()
    car_images.append(image) # add image to the car_images

  # sprite group for cars
  car_group = pygame.sprite.Group() # provides functionalities to handle and manipulate a collection of sprites efficiently

  high_score = load_high_score() # should add inside main game loop function
  ...
  while not quit_game: # Main event loop
      ...
      while game_over == True:
        ...
        pygame.draw.rect(screen, red, (0, 200, width,100)) # creates a rectangular object
        message ("You died! Play Again? Enter Y or N", white) 
        display_scores(score, black, high_score) 
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_n: # if user press the n, window will be closed
                    quit_game = True
                    game_over = False
                if event.key == pygame.K_y: # if user press the y, game will restart
                    game_loop()
      ...
      for event in pygame.event.get():
          ...
          if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT and player.rect.center[0] > left_lane:
              player.rect.x -= 100
            elif event.key == pygame.K_RIGHT and player.rect.center[0] < 400:
              player.rect.x += 100
      ...
      # add up to two cars
      if len(car_group) < 2: # len() is used to returns the number of items in an object
